...
There are currently {{if gt (len $erowid) 25}}25+{{else}}{{len $erowid}}{{end}} experience reports involving {{lower $v.Title}} on <a class=logo href=https://www.erowid.org/experiences/exp.cgi>Erowid</a>:
<ul>{{range slice $erowid 0 (min (len $erowid) 25) -}}
<li><a class=logo href='https://www.erowid.org/experiences/exp.php{{if empty .Id | not}}?ID={{.Id}}{{end}}' target="_blank">{{with .Title}}"<span class=report-title>{{.}}</span>"{{end}}{{with .Author}} by {{.}}{{end}}</a></li>
{{end}}
</ul>
{{end}}
</div>
...
<div class="reddit-reports">
{{if and $reddit (gt (len $reddit) 0)}}
There are currently {{len $reddit}} trip reports involving {{lower $v.Title}} on <a class=logo href=https://reddit.com/r/tripreports/>Reddit</a>:
<ul>{{range $reddit -}}
