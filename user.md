...
<div id=Infobox>
<table><tr>
<th contenteditable=false colspan="2" class=TitleMain>{{$v.Title}}</th>
</tr>
...
{{- if $v.Birth}}{{$cLen := toDate "2006-01" $v.Birth | ago | splitList "h" | first | add 240}}<tr><th contenteditable=false class="InfoboxLabel">Age</th>{{if $v.Deceased}}{{$dLen := toDate "2006-01" $v.Deceased | ago | splitList "h" | first}}{{$cLen = sub $cLen $dLen}}{{end}}<td id=userage contenteditable={{$v.Editable}} class=InfoboxData>{{if eq $v.Editable "true"}}{{$v.Birth}}{{else}}~{{divf (divf (div $cLen 24) 365 | mulf 10 | int) 10}} years{{if $v.Deceased}} (time of death){{end}}{{/* / ~{{div $cLen 24 }} <em>days</em> / ~{{$cLen}} <em>hours</em>*/}}</td></tr>{{end}}{{end}}
...
<input type=button id=editsubmit value="Publish changes" /><a id=editcancel style="color: var(--color-destructive) !important; font-weight: bold;" href="/user/{{$v.Title}}">Cancel</a>
</div>
<div style="border: 1px solid var(--border-color-base);" id=jsonoutput></div>
{{end}}
